...
const DataPlayerInfo = ({ action, status, target }) => {
  ...
  if (status && minimizar) {
    ...
    return <>
     <div className="dataplayerInfo-div-btn-icon">
        <button className="dataplayerInfo-iconthree"><FiMinimize2
          className="dataplayerinfo-Icontwo"
          onClick={() => {
            setMinimizar(true);
          }}
       ...
       <div className='dataplayerindfo-div-properties dataplayerInfo-div-minor'>
            ...
            {info.table.map(card => {
                if(card.type === 'property' && table[card.id].owner === dataPlayers[target].username){
                    return <div className='dataplayerinfo-div-name'>
                      <p className={`dataplayerinfo-color-${table[card.id].color}`}>{card.name}</p>  
                    </div>
                }else{
            ...
            <div>
            <p className='dataplayerindfo-p-properties'>Cards:</p>
            {/* {dataPlayers.map(cards =>{
                return <div>
                  <p>{cards}</p>
                </div>
            })} */}
